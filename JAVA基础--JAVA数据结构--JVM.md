...
**2. 默认散列表长度是16，大于数据负载因子后扩容时原始长度x2,，最消耗性能的点就出现了：原数组中的数据必须重新计算其在新数组中的位置，并放进去，这就是resize，所以使用的时候最好直接指定，防止资源浪费**
...
**取消了segment数组，直接用table保存数据，锁的粒度更小，减少并发冲突的概率。HashEntry 列表改为 一个一个的Node。由segment锁改为锁node。
