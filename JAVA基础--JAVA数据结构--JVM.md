...
1. 将所有泛型类型参数替换为其限定类型（如果没有限定类型，则替换为Object）。例如，对于`List<String>`，泛型类型参数`String`将被替换为`Object`，变成`List<Object>`（但实际上，在字节码中并不会显式地写成`List<Object>`，而是直接写成`List`，因为类型参数已被擦除）。
